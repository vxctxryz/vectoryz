...
_WIKI_API_OPENSEARCH_TEMPLATE = (
    "https://{lang}.wikipedia.org/w/api.php?"
    "action=opensearch&search={q}&limit={limit}&namespace=0&format=json"
)


def search_wikipedia_topic(query: str, lang: str = "de", limit: int = 3,
                            timeout: float = 5.0) -> list:
    """Wikipedia opensearch — query → ranked matching article titles.

    Per [[p3_knowledge_question_landed]] P3.5: for yes/no historical-fact
    questions, naive search-keyword extraction misses domain-specific
    article-titles ("motorsport olympic" → search returns generic IOC-info,
    NOT "Water motorsports at the 1908 Summer Olympics"). Wikipedia
    opensearch by relevance returns the right article-titles to fetch.

    Returns:
        list of {title, description, url} for top matches, or empty list.
    """
...
def search_and_fetch_summary(query: str, lang: str = "de",
                              max_articles: int = 2, timeout: float = 5.0) -> list:
    """Two-step: opensearch for titles → fetch summary for top-N. Returns
    list of summary-dicts (same shape as fetch_wiki_summary). Used by P3.5
    query-expansion for verifiable-fact questions.
    """
    titles_info = search_wikipedia_topic(query, lang=lang, limit=max_articles + 2,
                                          timeout=timeout)
    if not titles_info:
        return []
    summaries = []
    for t_info in titles_info[:max_articles]:
        title = t_info["title"]
        # Skip disambiguation-only titles for direct-fetch
        summary = fetch_wiki_summary(title, lang=lang, timeout=timeout)
