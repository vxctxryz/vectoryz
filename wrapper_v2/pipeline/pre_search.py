...
_GREETING_PATTERNS = re.compile(
    ...
    r"danke|merci|tschüss|bis\s+später|bis\s+bald|gerne|ok\b|alles\s+gut|"
    r"nice|cool|super|prima|läuft|geil|krass|sehr\s+gut|ja\b|nein\b|ahoi|"
    r"yo+\b|sup\b|good\s+(?:morning|evening|day))",
    re.IGNORECASE,
)

# Proper-noun heuristic — German capitalizes all nouns + names. Look for
# CamelCase or capitalized-word-not-at-start as proper-noun indicator.
_PROPER_NOUN_HINT = re.compile(r"(?<!^)(?<![\.\!\?]\s)[A-ZÄÖÜ][a-zäöüß]{2,}")


def _is_fast_path_eligible(message: str) -> bool:
    """Heuristic: skip LLM-classifier + web-search for simple queries.

    Eligible when:
    - Greeting-pattern at start (proper-noun-friendly, e.g. "hallo Claude")
    - OR short (<60 chars) AND no question-mark AND no proper-noun

    NOT eligible when:
    - URL present → user wants fetch
    - Question-mark → likely info-question
    - Capitalized-noun mid-sentence → likely entity-question (Piaget, Schworm)
    - Long (≥60 chars) → likely complex
    """
    if not message:
        return False
    m = message.strip()
    if not m:
        return False
    # URL → must fetch
    if _URL_PATTERN.search(m):
        return False
    # Greeting opener → fast-path (greetings can contain proper-noun after,
    # e.g. "hallo Claude" — still fast-path-class)
    if _GREETING_PATTERNS.match(m):
        return True
    has_question = ("?" in m) or ("¿" in m)
    has_proper_noun = bool(_PROPER_NOUN_HINT.search(m))
    if has_question or has_proper_noun:
        return False
    # Short + no question + no proper-noun
    if len(m) < 60:
        return True
    return False
...
def classify_needs_search(message: str) -> dict:
    ...
    # P1-Fix-1 fast-path: skip LLM when query is clearly-simple.
    # URLs always force LLM-engagement (handled below).
    if not user_urls and _is_fast_path_eligible(message):
        return {
            "needs_search": False,
            "topic": "",
            "reason": "fast-path-heuristic (simple-query, skip LLM-classifier)",
            "user_urls": [],
            "_fast_path": True,
        }

    # 2026-05-22 P1-Fix-2 (lite): REGEX-CLASSIFIER SHORT-CIRCUIT.
    # If fringe-classifier OR knowledge-question-classifier fires, we
    # already know search is needed (or at least not blocked) — skip the
    # 5-15s LLM-classifier-call. Regex is deterministic-cheap, LLM-call
    # is expensive-cold-start. For fringe (thestatica/wassergedächtnis/
    # NLP-in-edu/...) and yes_no_historical (motorsport/Piaget/...) we
    # bypass LLM entirely. Plain factual queries still hit LLM.
    try:
        from wrapper_v2.pipeline import fringe_classifier as _fringe
        _matched_fringe = _fringe.detect_fringe_terms(message)
        if _matched_fringe:
            return {
                "needs_search": True,
                "topic": " ".join(_matched_fringe[:3])[:200],
                "reason": (f"fringe-shortcut: matched terms "
                           f"{_matched_fringe[:3]}"),
                "user_urls": user_urls,
                "_classifier_shortcut": "fringe",
            }
    except Exception:
        pass
    try:
        from wrapper_v2.pipeline import knowledge_question_classifier as _kqc
        _kq_cls, _kq_force = _kqc.classify_query(message)
        if _kq_force:
            return {
                "needs_search": True,
                "topic": _kqc.expand_search_keywords(message, _kq_cls),
                "reason": f"kq-shortcut: class={_kq_cls}",
                "user_urls": user_urls,
                "_classifier_shortcut": "knowledge_question",
            }
    except Exception:
        pass
...
def classify_and_fetch(message: str, max_snippets: int = 4) -> Optional[dict]:
    ...
    # 2026-05-22 P3 — KNOWLEDGE-QUESTION-CLASSIFIER OVERRIDE.
    # If query is yes/no-historical or superlative-fact, FORCE needs_search=True
    # regardless of LLM-classifier verdict. Per [[fact_checker_layer4_doctrine]]
    # V2/V3 spirit + motorsports_olympic_v1 baseline 2026-05-22 fail (wrapper
    # confidently said "Motorsports nicht teilgenommen" missing 1908 motorboating).
    # Regex-classifier catches the query-class deterministically, then existing
    # search-infra fetches snippets. Smartfaul: cheap regex > expensive search-
    # routing-mistake.
    try:
        from wrapper_v2.pipeline import knowledge_question_classifier as _kqc
        _kq_class, _kq_force = _kqc.classify_query(message)
        if _kq_force:
            decision = dict(decision) if decision else {}
            if not decision.get("needs_search"):
                decision["needs_search"] = True
                decision["forced_by"] = f"knowledge_question_classifier:{_kq_class}"
                # If LLM didn't extract topic, use our cleaned keyword
                if not decision.get("topic"):
                    decision["topic"] = _kqc.expand_search_keywords(message, _kq_class)
    except Exception:
        pass
    ...
    # 2026-05-22 β — FRINGE-PRE-CLASSIFIER (highest priority, even above disambig).
    # When user query contains fringe-science / pseudoscience terms (Thestatica,
    # Perpetuum Mobile, Wassergedächtnis, Chemtrails, etc.), inject a HIGH-PRIORITY
    # directive demanding epistemic-status flagging. This catches the thestatica-
    # class failure (wrapper accepted fringe premise + listed BNetzA-steps) BEFORE
    # generation, not via retry-corrective (which proved insufficient per
    # [[p1_audit_recalibration_landed]] thestatica 0/2 retry-content-fix-rate).
    # Per smartfaul-doctrine: regex-keyword pre-pass is cheap + reliable; LLM
    # retry-correctives fight against "be helpful" pretraining and lose often.
    try:
        from wrapper_v2.pipeline import fringe_classifier as _fringe
        _fringe_matched, _fringe_directive = _fringe.check_and_build(message)
        if _fringe_directive:
            fetched["context_block"] = (
                _fringe_directive + "\n\n" + (fetched["context_block"] or "")
            )
            fetched["fringe_matched"] = _fringe_matched
    except Exception:
        pass
    # 2026-05-22 P3-Part-B — KNOWLEDGE-QUESTION DISCIPLINE-DIRECTIVE.
    # For yes/no-historical and superlative-fact queries, force-search alone
    # is insufficient — model still trusts prior over snippets. Inject a
    # discipline-directive that requires response to be conditioned on
    # snippets, with explicit "ich kann das aus Quellen nicht bestätigen"
    # when snippets insufficient. Catches the motorsports-class failure
    # (wrapper confidently "Motorsports nicht teilgenommen" missing 1908).
    try:
        from wrapper_v2.pipeline import knowledge_question_classifier as _kqc2
        _kq_class2, _kq_force2 = _kqc2.classify_query(message)
        if _kq_force2:
            _kq_directive = _kqc2.build_discipline_directive(_kq_class2)
            if _kq_directive:
                fetched["context_block"] = (
                    _kq_directive + "\n\n" + (fetched["context_block"] or "")
                )
                fetched["knowledge_question_class"] = _kq_class2
            # 2026-05-22 P3.5 — Wikipedia opensearch query-expansion.
            # Direct Wikipedia article-title lookup via opensearch API.
            # Partial-coverage: works when Wikipedia article-title contains
            # query-keywords (Eisstockschießen, Thestatica/Testatika). Misses
            # when canonical-article-title uses different terminology
            # (motorsport→Wassersport). Worth shipping — covers many cases.
            try:
                from wrapper_v2.pipeline import wiki_wortwolke as _ww2
                _expanded_kw = _kqc2.expand_search_keywords(message, _kq_class2)
                _wiki_summaries = []
                # Try DE first (operator's primary audience), fallback EN
                for _lang in ("de", "en"):
                    _wiki_summaries = _ww2.search_and_fetch_summary(
                        _expanded_kw, lang=_lang, max_articles=2, timeout=5.0
                    )
                    if _wiki_summaries:
                        break
                if _wiki_summaries:
                    _verif_lines = ["⚠ VERIFIED-WIKIPEDIA-CONTEXT (höchste Priorität, "
                                    "Quellen-Anker für die Antwort):"]
                    for _s in _wiki_summaries:
                        _verif_lines.append(
                            f"  • {_s.get('title','')}: "
                            f"{(_s.get('extract','') or '')[:500]}"
                        )
                        if _s.get('url'):
                            _verif_lines.append(f"    URL: {_s['url']}")
                    _verif_lines.append("")
                    _verif_lines.append(
                        "DISZIPLIN: konditioniere deine Antwort auf diese "
                        "Wikipedia-Quellen-Ankr. Wenn die Wikipedia-Daten der "
                        "User-Prämisse oder deinem Modell-Prior widersprechen, "
                        "VERTRAUE DEN WIKIPEDIA-QUELLEN über das Modell-Prior."
                    )
                    fetched["context_block"] = (
                        "\n".join(_verif_lines) + "\n\n" +
                        (fetched["context_block"] or "")
                    )
                    fetched["wikipedia_verified"] = [
                        {"title": _s.get("title"), "url": _s.get("url")}
                        for _s in _wiki_summaries
                    ]
            except Exception:
                pass
